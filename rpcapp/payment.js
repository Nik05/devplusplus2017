...
const payment = {
    ...
    createInvoice(satoshi, content, cb) {
        ...
        bitcoin.getNewAddress((err, response) =>{
            ...
            model.invoice.create(satoshi, content, ad, cb);
        });
            ...
            model.payment.findByAddr(invoice.addr, (pmtErr, payments) => {
                ...
                async.eachSeries(
                        ...
                        bitcoin.getTransaction(payment.txid, (gtErr, response) => {
                            ...
                            if (!blockhash){
                                ...
                                model.payment.setStatus(payment._id, 'reorg', (err) =>{
                                    ...
                                    }, 'a reorg occured', asyncCallback);
                                });
                                return;
                            }
                            const isFinal = confirmations >= config.requiredConfirmations;
                            if (minConfirms > confirmations){
                                minConfirms = confirmations;
                            }
                            if (maxConfirms < confirmations){
                                maxConfirms = confirmations;
                            }
                            totalAmount += amountSatoshi;
                            if (isFinal){
                                finalAmount += amountSatoshi;
                            }
                            model.payment.setStatus(
                                payment._id,
                    ...
                    () => {
                        ...
                        const confirmations = Math.min(minConfirms, maxConfirms); // How many confirmations does this invoice have? In case of multiple payments, how should we think about this?
                        const pendingAmount = totalAmount - finalAmount; // Only unconfirmed amount
                        const threshold = config.thresholdSatoshi; 
                        const finalRem = invoice.amount - finalAmount; // From the confirmed amount, how much is unpaid?
                        const totalRem = invoice.amount - totalAmount; // From the total amount paid, how much is unpaid?
                        const finalMatch = Math.abs(finalRem) < threshold; // After confirmation, if the paid amount is exact (true) or not (false)
                        const totalMatch = Math.abs(totalRem) < threshold; // Unconfirmed + Confirmed
